...
const StudentEntry = () => {
  ...
  const handleManualSubmit = async (e) => {
    ...
    // Additional validation for passedOutYear
    const currentYear = new Date().getFullYear();
    if (formData.passedOutYear < 1950 || formData.passedOutYear > currentYear) {
      toast.error('Please enter a valid passed-out year.');
      return;
    }
    ...
    try {
      ...
      setFormData({
        ...
        address: '',
        skillSet: '',
        passedOutYear: '', // Clear the passedOutYear field
      });
    } catch (error) {
      console.error('Error adding document: ', error);
      toast.error('Error saving data');
